...
hourExclusions = {
    'Saturday' : [23, 0, 1, 2, 3, 4, 5],
    'Sunday' : [0, 1, 2, 3, 4, 5, 6, 7, 8, 18, 19, 20, 21, 22, 23],
    'default' : [0, 1, 2, 3, 4, 5, 23]
}
...
for i in range(1, 13):
    hourlyWeather[i] = {}
    for j in range(0, 24):
        hourlyWeather[i][j] = [0, 0, 0, 0] #good, fair, ifr, not flyable
...
for path in jsonPaths:
    ...
    for point in data:
        try:
            ...
            if dayOfWeek in hourExclusions:excludeKey = dayOfWeek
            else: excludeKey = 'default'
            if hour in hourExclusions[excludeKey]: continue
            restriction = point[1][0]
            if hour == 5 and restriction in goodWeather:
                print(restriction)
            if restriction in goodWeather:  hourlyWeather[month][hour][0] += 1
            elif restriction in fairWeather:  hourlyWeather[month][hour][1] += 1
            elif restriction in ifr:  hourlyWeather[month][hour][2] += 1
            elif restriction in notFlyable:  hourlyWeather[month][hour][3] += 1
        except:
            pass

finalWeather = {}
for month in hourlyWeather.keys():
    finalWeather[month] = {}
    for hour in hourlyWeather[month]:
        if hour in [2, 3, 4, 5]: continue #cut out closed hours
        total = sum(hourlyWeather[month][hour])
        try:
            finalWeather[month][hour] = [round(hourlyWeather[month][hour][0] / total, 3), round(hourlyWeather[month][hour][1] / total, 3), round(hourlyWeather[month][hour][2] / total, 3), round(hourlyWeather[month][hour][3] / total, 3)]
        except:
            
            print('error: ', hourlyWeather[month][hour])
...
for month in finalWeather.keys():
    graphedWeather = finalWeather[month]
    hours = list(graphedWeather.keys())
    x = list(range(len(hours)))
    values = list(graphedWeather.values())
    ...
    plt.xlabel('Hour')
    plt.ylabel('Value')
    plt.title(monthDict[month] + ': Weather Categories By Hour')
    plt.xticks(x, hours)  # Ensure labels 1–12
    plt.legend()

    plt.tight_layout()
    plt.savefig('Graphs/ByHourByMonth/' + str(month) + '-' + monthDict[month] + ':ByHour')
    #plt.show()
